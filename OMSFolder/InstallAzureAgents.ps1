...
$OMSFolder = 'C:\OMSFolder'
$MMAFile = $OMSFolder + "\" + $MMAAgentFile
$DependencyAgentFile = $OMSFolder + "\" + $DependencyAgent
$Shared = "\\<DomainJoinedServer/AzureMigrateServer>\OMSFolder" # Use Create Shared Folder With Read Access to Everyone
...
else
{
	...
	Copy-Item -force $SharedMMA -Destination $OMSFolder -verbose
	Write-Host "done!" -ForegroundColor Green
}
...
else
{
	...
	Copy-Item -Force $SharedDependencyAgent -Destination $OMSFolder -verbose
	Write-Host "done!" -ForegroundColor Green
